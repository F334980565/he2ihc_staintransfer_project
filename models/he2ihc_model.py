...
class HE2IHCModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_discret', type=float, default=0.0, help='weight for discret loss')
    ...
    def backward_G_A(self):
        ...
        # GAN loss D_A(G_A(A))
        self.loss_G_GAN = self.criterionGAN(self.netD_A(self.fake_B), True)
        self.loss_G_lpips = self.criterionLpips(self.fake_B, self.real_B).mean() * self.opt.lambda_lpips
        # 把改动的L1全加上去
        self.loss_G_L1 = self.criterionL1(self.fake_B, self.real_B)
        self.loss_G_T, self.loss_G_discret = self.criterionT(self.real_B, self.fake_B)
        # 加个对应的consistency loss

        self.loss_G_A = (self.loss_G_GAN*self.opt.lambda_GAN + self.loss_G_lpips*self.opt.lambda_lpips + 
