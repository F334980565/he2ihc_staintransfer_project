...
class HE2IHCconditionModel(BaseModel):
    """
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_lpips', type=float, default=1.0, help='weight for VGGlpips loss')
            parser.add_argument('--lambda_L1', type=float, default=0.0, help='weight for prediction loss')
            parser.add_argument('--lambda_T', type=float, default=1.0, help='weight for consistency loss')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'G_GAN', 'G_lpips_A', 'G_L1', 'G_T', 'G_discret', 'G_identity'] #加了个G_consist
        ...
        if self.isTrain:
            self.model_names = ['G_A', 'D_A', 'D_A_condition']
        ...
        if opt.phase == 'test':
            if not opt.classifier_path == None:
                self.netF_A = networks.HE_resnet50().to(self.device)
                state_dict = torch.load(opt.classifier_path)
                self.netF_A.load_state_dict(state_dict)
                self.netF_A.requires_grad_(False).eval()
        
        if self.isTrain:  # define discriminators
            self.netD_A = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            
            self.netD_A_condition = networks.define_D(opt.input_nc + 1, opt.ndf, opt.netD,
                    opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.optimizer_D_A = torch.optim.AdamW(itertools.chain(self.netD_A.parameters(), self.netD_A_condition.parameters()), lr=opt.lr_D_A, betas=(opt.beta1, 0.999))
            self.optimizers.append(self.optimizer_G_A)
            self.optimizers.append(self.optimizer_D_A)
            
        self.map_tensor = torch.tensor([1, -1, 0]).to(self.device) #唉，又有历史遗留了
    ...
    def set_input(self, input):
        ...
        self.predict = input['label'].to(self.device)

        self.real_B_conditioned = self.get_conditioned(self.real_B, self.predict)
    ...
    def forward(self):
        ...
        self.real_A_conditioned = self.get_conditioned(self.real_A, self.predict)
        self.fake_B = self.netG_A(self.real_A_conditioned)  # G_A(A)
        self.fake_B_conditioned = self.get_conditioned(self.fake_B, self.predict)
        self.identity_B = self.netG_A(self.real_B_conditioned)
        
    def forward_test(self):
        if not self.opt.classifier_path == None:
            predict = self.netF_A(self.real_A)
            predict = self.map_tensor[predict]
        else:
            predict = torch.zeros(self.real_A.shape[0],)
        self.real_A_conditioned = self.get_conditioned(self.real_A, predict) #这个测试的时候改
        self.fake_B = self.netG_A(self.real_A_conditioned)  # G_A(A)
    ...
    def backward_D_A(self): #改动过的 
        ...
        self.loss_D_A_conditoned = self.backward_D_basic(self.netD_A_condition, self.real_B_conditioned, self.fake_B_conditioned)
    ...
    def backward_G_A(self):
        ...
        # GAN loss D_A(G_A(A))
        self.loss_G_GAN = (self.criterionGAN(self.netD_A(self.fake_B), True) + self.criterionGAN(self.netD_A_condition(self.fake_B_conditioned), True)) * 0.5
        # 加个对应的lpips
        ...
        Tloss, Tloss_discret = self.criterionT(self.real_B, self.fake_B)
        self.loss_G_T = Tloss * self.opt.lambda_T
        self.loss_G_discret = Tloss_discret * self.opt.lambda_discret
        self.loss_G_identity = self.criterionL1(self.real_B, self.identity_B) * self.opt.lambda_identity

        self.loss_G_A = self.loss_G_GAN + self.loss_G_lpips_A + self.loss_G_L1 + self.loss_G_T + self.loss_G_discret + self.loss_G_identity
    ...
    def get_conditioned(self, image, condition, test=False):
