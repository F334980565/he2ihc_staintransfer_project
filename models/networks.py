...
def get_scheduler(optimizer, opt):
    ...
    """
    if opt.lr_policy == 'linear': #linear是它自己写的学习率策略，别的都是调库
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + opt.epoch_count - opt.n_epochs) / float(opt.n_epochs_decay + 1) #opt.epoch_count > opt.n_epochs的时候才开始，即第n_epochs时才开始降低学习率，到n_epochs + n_epochs_decay的时候学习率降到0，训练结束
            return lr_l
...
class TLoss(nn.Module):
    ...
    def discret_function(self, tensor, params = [1.85, 0.5]):
        positive_thres, background_thres = params
        summed_tensor = torch.sum(tensor, dim=1)
        discret_tensor = torch.where(summed_tensor > positive_thres, 1.0,
                                    torch.where((summed_tensor <= positive_thres) & (summed_tensor >= background_thres), 0.0, -1.0))
        return discret_tensor
...
class Self_Attn(nn.Module):
    ...
    def forward(self,x):
        ...
        """
        m_batchsize, C, width, height = x.size() #你的H和W和别人不一样啊？搞毛？
        proj_query = self.query_conv(x).view(m_batchsize,-1,width*height).permute(0,2,1) # B,C,H*W -> B,H*W,C?
        proj_key =  self.key_conv(x).view(m_batchsize,-1,width*height) # B,C,H*W
        energy =  torch.bmm(proj_query,proj_key) # 矩阵乘法，应该乘出一个 B,N,N
...
class UnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        if num_downs >= 10:
            ...
            k = 8
        unet_block = UnetSkipConnectionBlock(ngf * k, ngf * k, input_nc=None, submodule=None, norm_layer=norm_layer, innermost=True)  # add the innermost layer
        for i in range(num_downs - 5):          # add intermediate layers with ngf * 8 filters
            unet_block = UnetSkipConnectionBlock(ngf * k, ngf * k, input_nc=None, submodule=unet_block, norm_layer=norm_layer, use_dropout=use_dropout)
        # gradually reduce the number of filters from ngf * 8 to ngf
        unet_block = UnetSkipConnectionBlock(ngf * 4, ngf * k, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
